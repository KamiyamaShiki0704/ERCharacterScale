...
当前源码为 **2.54.0-rc.5 候选版本**，运行标识为 `er-2.54-configurable-units-rc5`。
rc.5 用按页权限查询替换高频元数据检查中的整段内存扫描，修复 rc.4 在大块驻留内存上的高开销；默认构建继续关闭日志与诊断采集。保留已实测 c9520 缩小正常、布料无抽动的 rc.3 模型匹配行为；本版性能和外观仍需游戏验收。
...
- 本地玩家、c0000 本地 NPC，以及 **非 c0000 的 EnemyIns 角色模型**；没有 c0000 模型限制。
- `target = "enemy"` 保留兼容名称，表示支持的本地非玩家单位，包括敌人、友方、调试生成单位和召唤物。默认不查询阵营；联机/远程玩家、幽灵、坐骑仍排除。
...
替换 DLL 旁的 `ERCharacterScale.toml`：玩家继续使用旧映射，所有支持的本地非玩家单位（含友方和召唤物）
