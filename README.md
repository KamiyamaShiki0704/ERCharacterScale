...
[下载 v2.54.0-rc.5](https://github.com/KamiyamaShiki0704/ERCharacterScale/releases/tag/v2.54.0-rc.5) · [配置说明](docs/CONFIGURATION.md) · [版本记录](CHANGELOG.md)

## 功能

- 玩家和非玩家单位分别配置，倍率范围为 **0.5～3.0**，相对于各单位自己的原始大小。
- 支持玩家、c0000 本地 NPC 和非 c0000 角色模型。
- 按 `cxxxx` 模型编号、NPC 参数编号或事件实体编号筛选，分别设置不同倍率。
- 使用单位自身的 SpEffect 触发缩放，或通过 `constant` 模式始终应用倍率。
- 保留布料模拟；默认发布 DLL 不输出日志。

## 安装

适用于 **Windows x64、离线 Elden Ring WW2.7.1.0 / game1.17.1**。其他程序版本需要另行验证。

1. 从 [Release 页面](https://github.com/KamiyamaShiki0704/ERCharacterScale/releases/tag/v2.54.0-rc.5) 下载 `ERCharacterScale-v2.54.0-rc.5-windows-x64.zip`。
...
玩家缩放、体型切换及 c9520 场景已完成实测，布料表现正常。其他模型、Boss 阶段及特殊动作仍需分别验证。

联机/远程玩家、幽灵和坐骑不在支持范围内。若多个单位共用可修改的布料数据却要求不同倍率，这组单位会保持原始大小；未知模型布局也可能保持原始大小。功能不保证伤害、移动速度、抓取对位或所有攻击判定随倍率变化。
