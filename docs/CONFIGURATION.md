...
适用于 2.54.0-rc.6。默认 DLL 名称为 `ERCharacterScale.dll`。文件是 DLL 同目录的 `ERCharacterScale.toml`，
...
| `scale` | 大于 0 的有限数值；不设固定上下限，1.0 为该单位原始大小 |
...
倍率可填写 `0.1`、`0.25`、`4.0`、`10.0` 等；玩家、非玩家以及两种模式使用同样的规则。
